...
{              graphics routine all in one go.  This can be quite         }
{              computationaly intensive, so large arrays will display     }
{              with some tearing".                                        }
{                                                                         }
{                                                                         }
...
: GREEN-PEN     ( -- hpen )   { Create a colored pen style, thickness, RGB Color }
   PS_SOLID 1                 { Style and Thickness                              }
   000 16 LSHIFT
   255  8 LSHIFT OR
   000           OR           { Set RGB Color } 
...
: GREEN-BRUSH   ( -- hbrush )  { Sets brush color to full green hue }
   000 16 LSHIFT
   255  8 LSHIFT OR
   000           OR 
   CreateSolidBrush ;
...
100  constant world_x_size     { x size of the world matrix }

100  constant world_y_size     { y size of the world matrix }
...
: Life                            { The main Life routine goes here - we start with a random output }
  Random_Life_Array               { Replace this with your own Life routines to get them to display }
;
...
: Life>Rect                                                   { Turn Life Data into a series of rectangle         }
  ...
  World_y_Size 0 do
   World_x_Size  0 do
    Life_Array @ i World_y_Size j * + cells + @               { See if cell at (x,y) is alive                     }
